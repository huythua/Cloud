...
import { FiDatabase, FiClock, FiLock } from 'react-icons/fi'
...
export default function Login(){
  ...
  const submit = async (e) =>{
    ...
    try{
      ...
      setLoading(false)
      // Điều hướng tới trang chính sau 1s
      setTimeout(() => {
        navigate('/app')
      }, 1000)
    }catch(err){
      ...
      <div className="auth-form-card">
        <div className="auth-form-header">
          ...
          <p style={{margin: 0, fontSize: '14px', color: '#475569'}}>
            Chào mừng trở lại! Đăng nhập để quản lý database của bạn
          </p>
        </div>
        ...
        <form onSubmit={submit} className="auth-form">
          ...
          <button type="submit" className="btn-auth-primary" disabled={loading}>
            {loading ? (
              ...
              <>
                <FiLock size={16} style={{marginRight: '8px', display: 'inline-block'}} />
                Đăng nhập
              </>
            )}
          </button>
